...
For comprehensive architecture diagrams including detailed sequence flows, error isolation patterns, and event aggregation, see the [Production Features Architecture](./production-features.mdx#multi-namespace-sse-streaming) documentation.
...
For detailed architecture diagrams including complete error handling flow, circuit breaker state machine, error classification decision tree, and retry timing diagrams, see the [Production Features Architecture](./production-features.mdx#error-handling-with-retry-and-circuit-breaker) documentation.
...
For detailed architecture diagrams including caching layer architecture, request deduplication flow, rate limiting with token bucket algorithm, and Kubernetes API throttling, see the [Production Features Architecture](./production-features.mdx#caching-and-rate-limiting-architecture) documentation.
